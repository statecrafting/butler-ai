...
Run the local CI loop (`make ci`) and automatically fix what it surfaces. `make ci`
runs the same gate set as CI (`make spine` + `build` + `test` + `lint`): if it
passes locally, CI passes too.

## 1. Run the local CI loop

Invoke your CI composite (`make ci`) from the repo root. The Makefile (or
