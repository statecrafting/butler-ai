...
Install the CLI. `make setup` does this with the pinned version (`SPEC_SPINE_VERSION` in the Makefile) and is the preferred path; the raw commands are:

```bash
make setup                              # preferred: pinned version, then compile, index, verify
cargo install spec-spine-cli --locked   # from crates.io (needs a Rust toolchain)
...
**Burn-down:** {N unresolved units}  (from `make burndown`)
