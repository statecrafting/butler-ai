...
cargo +nightly udeps --workspace 2>/dev/null || cargo check --workspace 2>&1 | grep -E "never used|unused" ; npx --yes --no-install knip --no-exit-code 2>/dev/null
```
...
cargo clippy --workspace --all-targets 2>&1 | grep -E "unused|dead" || echo "(no unused lint findings)"
```
...
cargo +nightly udeps --workspace 2>/dev/null; pnpm -r exec depcheck 2>/dev/null || echo "(dependency audit tool not available; skipping)"
```
