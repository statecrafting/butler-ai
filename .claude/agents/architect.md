...
| Rust crates | `crates/{butler-core,butler-capture,butler-ocr,butler-llm}/` | Pure core + platform crates (specs 006-010, 009) |
| Desktop app | `apps/desktop/src-tauri/` (crate `butler-desktop`), `apps/desktop/src/` (SolidJS) | The Tauri shell and the overlay (specs 004, 011-014, 016) |
...
- **Verification** (the command that confirms the step: `make build`, `spec-spine compile`, `spec-spine lint`, `spec-spine couple`)
